...
function toSafeAiError(e) {
    ...
    if (status === 400) {
        const invalidKey = Array.isArray(details) &&
            details.some((d) => d?.reason === "API_KEY_INVALID" || /API key not valid/i.test(d?.message || ""));
        if (invalidKey)
            return { http: 500, message: "Server AI key is invalid. Update AI_API_KEY in Backend/.env." };
        return { http: 400, message: "Bad request to AI provider." };
    }
    if (status === 401 || status === 403)
        return { http: 500, message: "Server AI key is invalid or lacks access." };
    if (status === 429)
        return { http: 503, message: "AI rate-limited. Try again in a moment." };
    return { http: 502, message: "AI provider error. Try again." };
}
async function getGeminiKeyForUser(userId) {
