...
tripsRouter.post("/upsert", async (req, res) => {
...
tripsRouter.post("/sync", async (req, res) => {
    const parsed = bulkSchema.safeParse(req.body);
    if (!parsed.success)
        return res.status(400).json({ error: "Invalid input" });
    const userId = req.user.userId;
    await prisma.$transaction(parsed.data.trips.map((t) => prisma.trip.upsert({
...
tripsRouter.delete("/by-client/:clientId", async (req, res) => {
    const userId = req.user.userId;
